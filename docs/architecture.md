...
- 账号凭据加密保存，公众号使用 AppID/AppSecret，B站使用 passport 登录后获取的 Cookie 凭据；小红书当前主要依赖全局 myaibot API Key，账号入口仍待补齐。
...
| 小红书 | 已支持 | 已支持 | 真实发布代码路径已接入 myaibot API；账号状态、连接测试、状态轮询和真实联调仍需补齐 |
