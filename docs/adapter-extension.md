...
- `render(content_ir)`：把统一内容 IR 转为平台草稿。`profile.yaml` 在 Adapter 初始化时加载，不作为 `render` 参数传入。
...
- `credentials`：账号凭据。公众号来自 AppID/AppSecret；B站来自 Cookie；小红书当前主要依赖全局 myaibot API Key，尚未形成 per-account 连接入口。
...
| 小红书 | `options.title`、`options.content`、图片/视频公网 URL | 至少一张图片或一个视频，建议封面 | 真实发布代码路径已接入；账号状态、连接测试、状态轮询和联调待补齐 |
| 知乎 | `draft.title`、`draft.summary`、`draft.body`、`draft.tags` | 仅预览和模拟 | 暂不真实发布 |
